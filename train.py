...
class BusinessTrainer:
    ...
    def compute_loss(self, predictions: Dict, targets: Dict) -> Dict[str, torch.Tensor]:
        ...
        losses['total'] = (
            0.3 * losses['growth_absolute'] +
            0.2 * losses['growth_relative'] +
            0.2 * losses['growth_log'] +
            0.15 * losses['risk'] +
            0.15 * losses['kpi']
        )
...
def train_model(model: nn.Module, num_epochs: int = 200,
...
if __name__ == "__main__":
    ...
    model = BusinessModel()
    trained_model, checkpoint_path = train_model(model, num_epochs=200)
